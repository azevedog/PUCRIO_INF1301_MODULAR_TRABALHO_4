...
*		6		MCS  	9/dez/2016 inicio desenvolvimento instrumentacao
 ...
 *  Função: TAB  Verifica
 *  ****/
	TAB_tpCondRet TAB_Verifica ( TAB_tppTabuleiro tabuleiro, int* numErros ){

		int lin, col, condRet;
		int errosTotal = 0;
		int numLinhas, numColunas;
		int erroTesteAnterior = 0, erroTesteProximo = 0; erroCorrente = 0; erroTamanho = 0;

		if ( pTabuleiro != NULL ){
			CNT_CONTAR("TAB_tabuleiro_existe");

			numLinhas = tabuleiro->linhas;
			numColunas = tabuleiro->colunas;

			for(lin = 1; lin <= numLinhas; lin++){
				for(col = 1; col <= numColunas; col++){

					if(pTabuleiro->posicoes[lin-1][col-1] == NULL){
						CNT_CONTAR("TAB_anterior_com_erro")
					}
					else{
						CNT_CONTAR("TAB_anterior_sem_erro")
					}
				}
			}
...
TAB_tpCondRet TAB_CriarTabuleiro(int numColunas, int numLinhas,
	...
	for(lin =0; lin < numLinhas; lin++){
		...
		for(col = 0; col < numColunas; col++){
			...
			#ifdef _DEBUG
				(*novoTabuleiro)->tipo = "t"
				(*novoTabuleiro)->quantNos += 1;
			#endif
		}
	}	
